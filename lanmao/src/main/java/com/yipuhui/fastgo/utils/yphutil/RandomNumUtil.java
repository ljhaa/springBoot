...
public class RandomNumUtil {
	...
	private static void generaterandom() {
		if (allchars) {
			...
			if (upper.size() == Integer.valueOf(MagicParameterConstants.THREE)) {
			...
			} else if (upper.size() == Integer.valueOf(MagicParameterConstants.TWO)) {
			...
			if (upper.size() == MagicParameterConstants.THREE_INT) {
			...
			} else if (upper.size() == MagicParameterConstants.TWO_INT) {
...
// add the new random string
	...
	public static void setAlgorithm(String value) {
		algorithm = value;
		// a SecureRandom object is to be used
		secure = true;
	}
	...
	public static void setSingle(char[] chars, int value) {
		char[] tmp = chars;
		// set the array of chars
		single = tmp;
		// set the number of chars in array single
		singlecount = value;
		// set flag that single chars are in use
		singles = true;
	}
	...
	public static void setCharset(String value) {
		...
		if (value.compareTo(MagicParameterConstants.ALL) == 0) {
			allchars = true;
			// set allchars flag
			// all chars are to be used so there are no single chars to sort
			// through
			more = false;
		} else if ((value.charAt(1) == MagicParameterConstants.BAR.charAt(0)) && (value.charAt(0) != MagicParameterConstants.SLASH.charAt(0))) {
			// run through the ranges at most 3
			while (more && (value.charAt(1) == MagicParameterConstants.BAR.charAt(0))) {
	...
	public static String getRandomNum(String size) throws Exception{
		...
		RandomNumUtil.setCharset("0-9");
		//指定字符长度
		RandomNumUtil.setLength(size);
		//生成随机数
		RandomNumUtil.generateRandomObject();
		//得到随机数
		return RandomNumUtil.getRandom();
	}
	
	public static void main(String[] args) throws Exception {
		//指定字符内容为"0"-"z"，当然也可以是别的，比如"a-z"或"A-Z"
		RandomNumUtil.setCharset("0-z");
		//指定字符长度
		RandomNumUtil.setLength("20");
		//生成随机数
		RandomNumUtil.generateRandomObject();
		//得到随机数
		String str = RandomNumUtil.getRandom();
		//输出结果：PE68P<HD]tnmj0Fp8i0R
